...
-- Functional upgrades only; charge shot and targeting drones intentionally removed.
...
function XP.addXP(xpData, amount)
    ...
    xpData.current = xpData.current + amount
    xpData.totalXP = xpData.totalXP + amount
    local leveled = false
    while xpData.current >= xpData.toNextLevel do
        xpData.current = xpData.current - xpData.toNextLevel
        xpData.level = xpData.level + 1
        xpData.toNextLevel = math.max(xpData.toNextLevel + 1, math.floor(xpData.toNextLevel * 1.2))
        leveled = true
    end
    return leveled
end
...
XP.upgrades = {
    ...
    {id="firerate", name="+20% Fire Rate", description="Shoot faster with all weapons", apply=function(p) p.fireRateMultiplier=(p.fireRateMultiplier or 1)*0.8 end},
    {id="damage", name="+25% Damage", description="Deal more damage with all weapons", apply=function(p) p.damageMultiplier=(p.damageMultiplier or 1)*1.25 end},
    ...
    {id="pickup", name="Magnetic Pickups", description="Auto-collect nearby pickups", apply=function(p) p.magneticRange=math.max(p.magneticRange or 0,80) end},
    {id="piercing", name="Piercing Bullets", description="Bullets go through enemies", apply=function(p) p.piercingBullets=true end},
    {id="multishot", name="+1 Extra Projectile", description="Fire one additional bullet", apply=function(p) p.extraProjectiles=(p.extraProjectiles or 0)+1 end},
...
function XP.getRandomUpgrades(count)
    local shuffled={}
    for _,u in ipairs(XP.upgrades) do shuffled[#shuffled+1]=u end
    for i=#shuffled,2,-1 do local j=math.random(i); shuffled[i],shuffled[j]=shuffled[j],shuffled[i] end
    local result={}
    for i=1,math.min(count or 3,#shuffled) do result[#result+1]=shuffled[i] end
    return result
end
