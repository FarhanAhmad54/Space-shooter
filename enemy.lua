...
-- Enemy types with different stats and behaviors (matching asset names)
Enemy.types = {
    swarmer = {
        speed = 120,
        health = 10,
    ...
    sniper = {
        ...
        health = 50,
        radius = 14,
        color = {0.2, 0.8, 1}, -- Changed from Green to Cyan
        score = 25,
        xp = 25,
        shootCooldown = 2.5,
        imageName = "sniper"
    ...
    serpent = {
        ...
        shootCooldown = 0,
        serpentLength = 5,
        imageName = "swarmer"  -- Reuse swarmer image
    },
    teleporter = {
        speed = 0,  -- Doesn't move normally
        health = 150,
        radius = 13,
        color = {0.8, 0.3, 1},
        score = 35,
        ...
        imageName = "sniper"  -- Reuse sniper image
    },
    carrier = {
        speed = 30,
        health = 150,
        ...
        shootCooldown = 0,
        spawnCooldown = 4.0,
        imageName = "miniboss"  -- Reuse miniboss image
    },
    shielded = {
        speed = 70,
        health = 50,
        ...
        shootCooldown = 3.0,
        shieldHealth = 3,
        imageName = "turret"  -- Reuse turret image
    }
}
...
function Enemy.new(x, y, enemyType, enemyImage)
    ...
    self.y = y
    self.type = enemyType
    self.speed = typeData.speed
    self.radius = typeData.radius
    self.health = typeData.health
    self.maxHealth = typeData.health
    self.color = typeData.color
    self.score = typeData.score
    self.dead = false
    self.quality = "high"
    self.age = 0
    self.damage = 1
    ...
    -- Shooter specific
    self.shootTimer = typeData.shootCooldown or 0
    self.shootCooldown = typeData.shootCooldown or 0
    
    -- Special enemy properties
    if enemyType == "serpent" then
        self.trail = {}
        self.trailTimer = 0
    elseif enemyType == "teleporter" then
        self.teleportTimer = typeData.teleportCooldown
    elseif enemyType == "carrier" then
        self.spawnTimer = typeData.spawnCooldown
        self.spawnCooldown = typeData.spawnCooldown
    elseif enemyType == "shielded" then
        self.shieldHealth = typeData.shieldHealth
        self.maxShieldHealth = typeData.shieldHealth
    end
...
function Enemy.getRandomType(wave)
    -- Introduce new enemy types based on wave
    local availableTypes = {"swarmer"}
    
    if wave >= 2 then
        table.insert(availableTypes, "turret")
    end
    if wave >= 3 then
        table.insert(availableTypes, "sniper")
    end
    if wave >= 4 then
        table.insert(availableTypes, "bomber")
    end
    if wave >= 5 then
        table.insert(availableTypes, "miniboss")
    end
    -- NEW ENEMIES
    if wave >= 6 then
        table.insert(availableTypes, "serpent")
    end
    if wave >= 8 then
        table.insert(availableTypes, "teleporter")
    end
    if wave >= 10 then
        table.insert(availableTypes, "shielded")
    end
    if wave >= 12 then
        table.insert(availableTypes, "carrier")
    end
    
    return availableTypes[math.random(#availableTypes)]
end

function Enemy:setQuality(quality)
...
function Enemy:canShoot()
    if self.shootCooldown > 0 and self.shootTimer <= 0 then
...
function Enemy:takeDamage(damage)
    -- Shielded enemies take shield damage first
    if self.type == "shielded" and self.shieldHealth > 0 then
        self.shieldHealth = self.shieldHealth - damage
        if self.shieldHealth < 0 then
            self.health = self.health + self.shieldHealth  -- Overflow damage
            self.shieldHealth = 0
        end
    else
...
function Enemy:draw(playerX, playerY)
    -- Draw enemy with image if available
    if self.image then
        -- Neon glow effect with multiple layers
        local r, g, b = self.color[1], self.color[2], self.color[3]
        local pulse = math.sin(love.timer.getTime() * 3) * 0.2 + 0.8  -- Subtle pulse
        
        -- Outer glow layers (neon spread)
        love.graphics.setColor(r, g, b, 0.15 * pulse)
        love.graphics.circle("fill", self.x, self.y, self.radius + 10)
        
        love.graphics.setColor(r, g, b, 0.25 * pulse)
        love.graphics.circle("fill", self.x, self.y, self.radius + 7)
        
        love.graphics.setColor(r, g, b, 0.4 * pulse)
        love.graphics.circle("fill", self.x, self.y, self.radius + 4)
        
        -- Bright core (neon center)
        love.graphics.setColor(r, g, b, 0.6 * pulse)
        love.graphics.circle("fill", self.x, self.y, self.radius + 2)
        
        -- Draw the actual enemy image
        ...
        if self.type == "bomber" and playerX and playerY then
            ...
            local dist = util.distance(self.x, self.y, playerX, playerY)
            if dist < 150 then
                local glowIntensity = math.max(0, 1 - dist / 150)
                local pulse = math.sin(love.timer.getTime() * 15) * 0.5 + 0.5
                love.graphics.setColor(1, 0, 0, 0.6 * glowIntensity * pulse)
                love.graphics.circle("fill", self.x, self.y, self.radius + 12)
                -- Removed warning ring outline
            end
        end
        
        -- Darken color based on health
        r = r * (0.5 + healthPercent * 0.5)
        g = g * (0.5 + healthPercent * 0.5)
        b = b * (0.5 + healthPercent * 0.5)
        ...
        love.graphics.circle("fill", self.x - 3, self.y - 3, self.radius * 0.3)
    end
    
    -- Draw health bar
    if self.health > 0 and self.maxHealth > 1 then
        local barWidth = self.radius * 2.5
        local barHeight = 4
